...
export function createTransformer({
  ...
  const transformer: ts.TransformerFactory<ts.SourceFile> = (context) => {
    ...
    return (sourceFile) => {
      ...
      const transformStyledFunction = (binding: ts.VariableDeclaration | ts.ExportAssignment, node: ts.Expression) =>
        withConfig(node, [
          ...(displayName ? createDisplayNameConfig(getDisplayNameFromNode(binding, sourceFile)) : []),
          ...(ssr ? createIdConfig(getIdFromNode(binding, sourceRoot, ++lastComponentPosition, sourceFile)) : []),
        ]);
      ...
      const updateNode = <T extends ts.Node, D>(node: T, data: D | undefined, updateFn: (node: T, data: D) => T) =>
      ...
      const updateVariableDeclarationInitializer = (node: ts.VariableDeclaration, initializer: ts.Expression) =>
        context.factory.updateVariableDeclaration(node, node.name, node.exclamationToken, node.type, initializer);
